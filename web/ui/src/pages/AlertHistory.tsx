      ...
      {history && (
        <>
          ...
          <div className="pagination">
            ...
            <span className="page-info">{page * limit + 1} - {Math.min((page + 1) * limit, history.total)} / {history.total}</span>
            <button className="btn-sm" disabled={(page + 1) * limit >= history.total} onClick={() => setPage(p => p + 1)}>下一页</button>
          </div>
        </>
      )}
    </div>
