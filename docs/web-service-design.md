# monitorbeat Web 服务 + 前端设计（P3 + P4）

> 状态：MVP 实现 + 生产化逐步完善。数据路径选定 **对接 VictoriaMetrics**，前端 **React + Vite**。
> 当前版本：`v0.0.1`。已部署到 k3s（`sha-ade47cd`），通过 Traefik Ingress（TLS）对外暴露。
> P3 新增：Alert engine（阈值评估 + SQLite 持久化 + Email 通知），以及后端 API 查询代理 + 静态前端一体托管。
> P4 新增：Agent Registry（主动注册 + 心跳 + 版本发现），嵌入 monitorweb 单二进制。
...
- `monitorweb` 是一个独立 Go 二进制，作为 **VM PromQL 查询代理 + Agent Registry + 静态前端服务**。
- 前端是 React + Vite SPA，构建产物由 Go 直接托管。
- agent 端零代码改动（仅指标采集）；如需 agent 发现能力，加 `registry` 配置节即可。
...
## 10. Agent Registry（P4，设计阶段）

> 完整设计见 `docs/superpowers/specs/2026-07-17-p4-agent-registry-design.md`

### 10.1 动机

当前 monitorweb 通过 `max by(hostname)(cpu_usage)` 隐式发现 agent，无法获取版本、task 列表、实时在线状态。Agent Registry 提供主动注册机制填补这一缺口。

### 10.2 数据流

```
monitorbeat ──POST /api/v1/registry/heartbeat──▶ monitorweb Registry
  {hostname, version, tasks, ip, k8s_node}          │
                                                     ▼
                                              SQLite (agents 表)
                                                     │
...
alert 和 registry 共享同一 SQLite 文件，由 `db_path` 配置：

```yaml
db_path: /data/monitorweb.db  # 一个文件，多张表
# 表: alert_rules, alert_history, alert_states, agents
```
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/api/v1/registry/heartbeat` | agent 上报心跳（~200 字节）→ 204 |
| `GET` | `/api/v1/registry/agents` | 所有已知 agent（含 online 状态） |
| `GET` | `/api/v1/registry/agents?online=true` | 仅在线 agent |

### 10.5 前端增强

- Overview 页优先用 registry 数据源（准确在线状态），降级到 VM hosts
- HostCard 显示 `version` 标识 + `task` 标签徽章
