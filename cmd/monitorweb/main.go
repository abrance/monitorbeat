...
func main() {
	...
	// Build VM client; passed both to the API and to the evaluator.
	...
	// Start alert evaluator. The VM client satisfies alerts.VMQuerier
	// directly, so no adapter is needed.
	evaluator := alerts.NewEvaluator(alertStore, vmc, emailSender, cfg.Alert.EvalInterval)
	evaluator.Start()
